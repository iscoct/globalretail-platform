...
Three pitfalls hit during the end-to-end run on 2026-05-25.

### 6.1 `kubelogin` not on PATH after `az aks install-cli`
**Symptom:** `helm upgrade --install argocd ...` fails with
`Kubernetes cluster unreachable: ... exec: executable kubelogin not found ... kubelogin is not installed which is required to connect to AAD enabled cluster.`
**Cause:** `az aks install-cli` installs kubectl and kubelogin into per-user directories (`~/.azure-kubectl` and `~/.azure-kubelogin`) but does NOT modify Windows PATH. The kubeconfig generated by `az aks get-credentials` references `command: kubelogin` (no absolute path), so helm/kubectl try to find it via PATH and fail.
**Fix:** `install-argocd.ps1` defensively prepends both directories to `$env:PATH` at the start of the script if the EXEs are present there but PATH does not include them. Documented for posterity because the failure mode is opaque (helm just says "cluster unreachable" without explaining why).

### 6.2 The empty `global:` block in `values-argocd.yaml` crashes the chart's templates
**Symptom:** `helm install` fails with
`error calling include: ... executing "argo-cd.defaultTag" at <.Values.global.image.tag>: nil pointer evaluating interface {}.image`
**Cause:** I had a `global:` key in `values-argocd.yaml` followed only by comments. From Helm's POV that's `global: null`. The chart's `_common.tpl` (since ~v8.x) reads `.Values.global.image.tag` to compute the default tag for label values. Reading `.image.tag` from `null` is a nil-pointer template panic.
**Fix:** Remove the `global:` block entirely. Letting the chart's own defaults take over means the chart's `appVersion` is used. If you want to override a specific global value, define it properly with sub-keys — but don't leave a top-level key with no children.

**Lesson:** an empty YAML block is not "no override"; it's "explicitly null". With Helm, "no override" is "no key at all".

### 6.3 Distroless `nonroot` user with `runAsNonRoot: true` — must set numeric `runAsUser`
**Symptom:** Pods stuck in `CreateContainerConfigError`. `kubectl describe pod` shows
`Error: container has runAsNonRoot and image has non-numeric user (nonroot), cannot verify user is non-root`.
**Cause:** The distroless image's Dockerfile contains `USER nonroot` — that is the *string* nonroot, not a numeric uid. When `securityContext.runAsNonRoot: true` is set on a pod, the kubelet must verify *at create time* that the effective uid is non-zero, *before* the container's entrypoint runs. A string username cannot be validated without starting the container (since names resolve to uids inside the container). Kubernetes refuses to start the container.
**Fix:** Add an explicit `runAsUser: 65532` (and `runAsGroup: 65532`) to the container's securityContext. Distroless's nonroot is uid 65532; setting it numerically resolves the validation. The first apply of the corrected manifest rolls the deployment cleanly within seconds.

**Lesson:** when using distroless and `runAsNonRoot`, ALWAYS set `runAsUser` explicitly. The official distroless README documents this requirement, but it's easy to miss — and the failure mode (CreateContainerConfigError) is unintuitive without `kubectl describe`.
...
- [x] Iteration 2: end-to-end test — ArgoCD installed, root discovered child Application, sample-app pods Running, `/health` + `/version` endpoints reachable via Service
- [x] §6 Pitfalls filled with real incidents (3 hit during iteration 2)
- [ ] Iteration 3: teardown verification (helm uninstall + kubectl delete ns)
- [ ] Demo prepared for class

## Iteration log

### Iteration 1 — code drafted (2026-05-25)
Wrote `bootstrap/install-argocd.ps1` + `values-argocd.yaml`, the app-of-apps `root-app.yaml`, the child `applications/sample-app.yaml`, Kustomize base + dev overlay. Sub-README §1–§5, §7, §8 written; §6 marked TBD-AFTER-BUILD.

### Iteration 2 — end-to-end test (2026-05-25)
Three pitfalls hit (kubelogin PATH, empty `global:` block, distroless `runAsUser` numeric uid) — all documented in §6. After the fixes:
- `helm upgrade --install argocd argo/argo-cd` deployed ArgoCD into namespace `argocd` (7 pods: server, controller, repo-server, redis, dex-server, applicationset-controller, notifications). Even though we disabled dex and notifications in values, the chart still installs them — they just don't do anything. Cleaner trimming is a future iteration.
- `kubectl apply -f root-app.yaml` → root Application syncs against `gitops/applications/` → ArgoCD auto-creates the `sample-app` Application within ~10 s.
- After the distroless uid fix: deployment rolls cleanly. Both pods `Running` within ~30 s.
- `/health` returns `{"status":"ok","uptimeSeconds":N}` via `kubectl port-forward svc/sample-app 8081:80`. `/version` returns `{"name":"globalretail-sample-app","version":"dev","commit":"unknown"}` — `dev` because the image's APP_VERSION env var isn't set in the deployment (next iteration could wire CI to inject the SHA).
- Both Applications converge to `Synced + Healthy` in the ArgoCD UI.

**State at end of iteration 2:**
- AKS cluster has ArgoCD + sample-app running.
- ArgoCD watches `iscoct/globalretail-platform@main`, polls every ~3 minutes.
- Manual `kubectl edit` against sample-app gets reverted by `selfHeal: true` within a tick.
- Any push to `gitops/applications/` adds a new managed app; any push to `gitops/workloads/sample-app/**` rolls the deployment if the rendered manifest changed.
